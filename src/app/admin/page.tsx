...
export default function AdminDashboard() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (f.size > 8 * 1024 * 1024) {
      showToast("error", "File too large — maximum 8 MB.");
      return;
    }
    setFile(f);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-4 rounded-sm border shadow-xl animate-[fadeUp_0.3s_ease] ${toast.type === "success" ? "bg-[var(--color-bg-low)] border-green-500/30 text-green-400" : "bg-[var(--color-bg-low)] border-red-500/30 text-red-400"}`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-[var(--color-border)]/20 pb-5 gap-2">
        <div>
          ...
          <p className="font-sans text-[var(--color-faint)] text-xs tracking-widest uppercase mt-2">Manage product and lifestyle pictures</p>
        </div>
      ...
      <div className="bg-[var(--color-bg-low)] border border-[var(--color-border)]/20 rounded-sm p-4 md:p-8">
        ...
        <label className="block border-2 border-dashed border-[var(--color-border)]/40 hover:border-[var(--color-gold-mid)]/50 transition-colors rounded-sm p-8 md:p-12 flex flex-col items-center justify-center text-center cursor-pointer relative">
          <input ref={inputRef} type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={handleFileChange} disabled={uploading} />
          {preview ? (
        ...
        <div className="mt-6 flex items-center justify-end gap-4">
          {file && !uploading && (
            <button onClick={() => { setFile(null); setPreview(null); if (inputRef.current) inputRef.current.value = ""; }} className="flex items-center gap-2 font-sans text-[10px] uppercase tracking-widest text-[var(--color-faint)] hover:text-red-400 transition-colors">
              <X className="w-3.5 h-3.5" /> Clear
            </button>
          )}
          <button onClick={handleUpload} disabled={!file || uploading} className={`flex items-center gap-2 px-8 py-3 font-sans text-xs uppercase tracking-widest rounded-sm font-semibold transition-all ${!file || uploading ? "bg-[var(--color-bg-card)] text-[var(--color-faint)] cursor-not-allowed" : "btn-gold"}`}>
            {uploading ? <><Loader2 className="w-4 h-4 animate-spin" /> Uploading...</> : <><UploadCloud className="w-4 h-4" /> Upload Image</>}
          </button>
        </div>
      </div>
      ...
      {/* Gallery grid */}
      <div>
        <h2 className="font-sans text-[10px] uppercase tracking-[0.25em] text-[var(--color-gold-mid)] mb-6">Uploaded Images</h2>
        {loadingGallery ? (
            ...
            {uploadedImages.map((src, i) => (
              <div key={i} className="aspect-square rounded-sm overflow-hidden border border-[var(--color-border)]/20 relative group">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3">
                  <button onClick={() => navigator.clipboard.writeText(src)} className="text-[var(--color-gold)] border border-[var(--color-gold)]/40 px-4 py-2 font-sans text-[9px] uppercase tracking-widest rounded-sm hover:bg-[var(--color-gold)]/10 transition-colors">Copy URL</button>
                </div>
              </div>
            ))}
          </div>
