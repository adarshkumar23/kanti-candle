...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (!ab.byteLength) return NextResponse.json({ error: 'Empty body' }, { status: 400 });
    ...
    // Return URL — DB persistence is handled by the caller via /api/gallery
