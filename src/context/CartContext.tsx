...
const STORAGE_KEY = "kanti-cart";
const DISCOUNT_KEY = "kanti-discount";

export function CartProvider({ children }: { children: ReactNode }) {
  const [items, setItems] = useState<CartItem[]>([]);
  const [discount, setDiscountState] = useState<AppliedDiscount | null>(null);
  ...
  useEffect(() => {
    try {
      ...
      if (storedDiscount) setDiscountState(JSON.parse(storedDiscount));
    } catch {}
    setIsHydrated(true);
  }, []);
  ...
  const setDiscount = useCallback((d: AppliedDiscount | null) => {
    setDiscountState(d);
  }, []);
  ...
  // Recalculate discount amount when cart total changes
  ...
  // Update the discount object's amount when total changes
  const appliedDiscount = discount ? { ...discount, discountAmount: discountAmount } : null;
